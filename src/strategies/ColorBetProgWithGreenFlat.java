...
public class ColorBetProgWithGreenFlat {
	...
	public ColorBetProgWithGreenFlat(double cash, int turnsToWait,
			double startingBet, double MAX_EVEN, int greenWait, double MAX_OTHER) {
		this.cash = cash;
		this.MAX_OTHER = MAX_OTHER;
		this.greenWait = greenWait;
		this.startingBet = startingBet;
		this.MAX_EVEN = MAX_EVEN;
		this.cashReset = cash;
		this.mostCash = 0;
		this.colorWait = turnsToWait;

		this.colorProgWithGreenFlat();
	}

	private void colorProgWithGreenFlat() {
		...
		while (this.turnNumber < this.roulette.getNumberOfTurns()) {
			...
			if (nonGreenCount >= this.greenWait && this.hasGreenBet == false) {
				...
				// CONSECUTIVE BETS AFTER NONGREENS
